...
class ActivityController extends Controller
{
    ...
    {
        $this->check();
        $all_activity = DB::table('tbl_activity')
        ->join('tbl_category', 'tbl_activity.category_id', '=', 'tbl_category.category_id')->orderBy('tbl_activity.activity_id','desc')
         ->get();
    ...
    public function save_activity(Request $request)
    {
        ...
        if($get_image){
            ...
            DB::table('tbl_activity')->insert($data);
    ...
    public function unactive_activity($activity_id){
        $this->check();
        DB::table('tbl_activity')->where('activity_id',$activity_id)->update(['activity_status'=>1]);
        return Redirect::to('all-activity');
    }
    public function active_activity($activity_id){
        $this->check();
        DB::table('tbl_activity')->where('activity_id',$activity_id)->update(['activity_status'=>0]);
        return Redirect::to('all-activity');
    }
    public function edit_activity($activity_id){
        $this->check();
        $cate_activity = DB::table('tbl_category')->orderBy('category_id','desc')->get();
        $edit_activity = DB::table('tbl_activity')->where('activity_id',$activity_id)->get();
    ...
    public function update_activity(Request $request,$activity_id){
        ...
        if($get_image){
            ...
            $get_image->move('public/uploads/product',$new_image);
            $data['activity_image'] = $new_image;
            DB::table('tbl_activity')->where('activity_id',$activity_id)->update($data);
    ...
    public function delete_activity($activity_id){
        $this->check();
        DB::table('tbl_activity')->where('activity_id',$activity_id)->delete();
        return Redirect::to('all-activity');
    }
    //End Admin
    public function details_activity($activity_id){
        $cate_activity = DB::table('tbl_category')->where('category_status','0')->orderBy('category_id','desc')->get();
        $details_activity = DB::table('tbl_activity')
        ->join('tbl_category', 'tbl_activity.category_id', '=', 'tbl_category.category_id')->where('tbl_activity.activity_id',$activity_id)
         ->get();
        // foreach($show_activity as $key=>$show){
        //     $show_activity = $show->category_id;
        // }
        $related_activity = DB::table('tbl_activity')
        ->join('tbl_category', 'tbl_activity.category_id', '=', 'tbl_category.category_id')->where('tbl_activity.category_id',$activity_id)
         ->limit(4)->get();
